...
* ("type:tmdbId") - synced by the Worker's chained weekly
* streaming sync (see advanceStreamingSyncChain in worker.js),
* just read here. Value is { services, extended }: services is
* the short original-whitelist list the front badge and the
* main back note already used (Kanopy, Hulu, Disney+, Amazon
* Prime Video, Netflix, HBO Max, Peacock, Amazon Rental) -
* unchanged from before the Stream tab existed - extended is
* the wider list (see EXTENDED_STREAMING_PRIORITY in worker.js)
* shown only in the Stream tab. Either can be an empty array if
* nothing matched.
  */
...
* Same idea as wishlistStreaming above, but for titles you
* physically own that AREN'T also owned digitally (movie.digital
* is empty) - see catalogSyncNotDigitalList/syncCatalogStreamingTargets
* below for how the Worker learns which titles those are. Only
* ever has an "extended" list, stored directly under .services -
* there's no front-badge/main-back-note equivalent for owned
* titles, only the Stream tab.
  */

let catalogStreaming = new Map();
...
extended: Array.isArray(row.extended_services) ? row.extended_services : []
}
]
)
);
...
// =========================================================
// LOAD CATALOG STREAMING AVAILABILITY (owned, not digital)
// =========================================================

async function loadCatalogStreaming() {

try {

const response =
await fetch(
`${RESERVATIONS_API}/catalog-streaming`,
{
method: "GET",
cache: "no-store"
}
...
throw new Error(
`Catalog streaming endpoint returned ${response.status}`
);
...
console.error(
"Could not load catalog streaming availability:",
error
);

catalogStreaming = new Map();
...
* Tells the Worker which owned titles currently have no digital
* copy (movie.digital empty) - the Worker's weekly chained sync
* (advanceStreamingSyncChain) reads this list back to know what
* to check, since it has no access to movies.js itself (that's
* generated by a separate GitHub Action, never touches D1).
* Only actually posts when the list has changed since the last
* sync (comparing a simple fingerprint saved in localStorage) -
* movies.js only changes when the Action runs, so on a normal
* visit this does nothing at all, not even the one POST call.
  */

function syncCatalogStreamingTargets() {

const notDigitalItems =
movies
.filter(
movie =>
...
"catalogStreamingFingerprint"
) || "";

} catch (storageError) {

// Private-browsing/blocked storage - just skip the
// short-circuit and sync every visit instead of failing.
...
// Nothing to do - it'll just sync again next visit.
...
console.error(
"Could not sync catalog streaming targets:",
error
);
...
loadCatalogStreaming();

syncCatalogStreamingTargets();
...
* The non-Info panels a case-back tab can switch to - each one
* is a genuine sibling div of the three back-bands (see the
* HTML comments by modal-history-panel/modal-trailer-panel/
* modal-streaming-panel), shown one at a time by hiding the
* three back-bands and un-hiding whichever of these matches the
* clicked tab. Info itself isn't in this list - it's just "none
* of these are showing, the back-bands are."
  */

const CASE_BACK_TAB_PANEL_IDS =
{
history: "modal-history-panel",
trailer: "modal-trailer-panel",
streaming: "modal-streaming-panel"
};
...
Object.values(CASE_BACK_TAB_PANEL_IDS).forEach(
panelId => {
...
// =========================================================
// CASE BACK - TRAILER TAB
// =========================================================

/*

* Live, on-demand only - the whole point of putting this behind
* a tab instead of on the main back is that it costs nothing
* until someone actually asks for it. Cached per movie per modal
* session (trailerCache), same reasoning as
* tournamentHistoryCache above: flipping to Info and back to
* Trailer on the same movie shouldn't re-hit the Worker.
  */

const trailerCache =
{};

function paintTrailerPanel(
panel,
key
) {
...
panel.innerHTML =
`<p class="history-empty">No trailer found for this one.</p>`;
...
`<iframe src="https://www.youtube.com/embed/${key}" title="Trailer" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>` +
`</div>`;
...
panel.innerHTML =
`<p class="history-empty">Loading trailer...</p>`;

fetch(
`${RESERVATIONS_API}/trailer?tmdb_id=${encodeURIComponent(movieId)}&media_type=${encodeURIComponent(movie.type)}`
)
.then(
response => {
...
panel.innerHTML =
`<p class="history-empty">Couldn't load a trailer for this one.</p>`;
...
// =========================================================
// CASE BACK - STREAMING ("Stream") TAB
// =========================================================

/*

* Unlike the trailer tab, this reads from data the Worker
* already keeps in sync in the background (wishlistStreaming /
* catalogStreaming - see loadWishlistStreaming/
* loadCatalogStreaming) rather than fetching live on click, so
* it can also eventually back an advanced-search "available on"
* filter without re-checking hundreds of titles at search time.
  */
...
panel.innerHTML =
`<p class="history-empty">${message}</p>`;
...
panel.innerHTML =
`<p class="history-empty">Not currently streaming anywhere we track.</p>`;
...
.map(
service =>
`<span class="streaming-tab-chip">${service}</span>`
)
.join("") +
`</div>`;
...
* a much shorter panel (or back) leaves the visitor staring
* at blank space below wherever they'd scrolled to, even
* though the new view's real content is sitting correctly
...
const targetPanelId =
CASE_BACK_TAB_PANEL_IDS[view];

if (!targetPanelId) {

// "info" (or anything unrecognized) - show the three
// back-bands, hide every other panel.
